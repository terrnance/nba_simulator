...
public class NBAGame {
	...
	//Finds and sets the best player on each team
	private static void initializeHead2Head(String directory_name) {
		...
		for (int team_index= 0; team_index<teamcounter.size();team_index++) {
			...
			//Get the best player on every team
			getBestPlayer(team_index);
		}
		...
		private static String getBestPlayer(int team_index_given) {
		...
		// Also calculates the average rating of player's in the rotation
		...
		private static String getTeamAverage(Team team_given,boolean included_injured) {
			...
			float team_rotation_number_players=0;
			float team_rotation_total_ratings = 0;
			...
			for(int player_index=0;player_index<team_given.leng();player_index++) {
				...
				if (team_given.getPlayer(player_index).getRating() > 0) {
					...
					if(!team_given.getPlayer(player_index).getInjury() || included_injured) {
						...
						//Calculate the averages of the player who are in the rotation
						if(team_given.getPlayer(player_index).getRating()>65) {
							team_rotation_total_ratings+=team_given.getPlayer(player_index).getRating();
							team_rotation_number_players++;
						}
			...
			float team_rotation_average= team_rotation_total_ratings/team_rotation_number_players;
			...
			team_given.setAvg(team_rotation_average);
			return average;	
		}
		...
		private static int findXPlayer(Team team_given) {
			...
			while (team_given.getPlayer(wild_card_player_index).getInjury()&&team_given.getPlayer(wild_card_player_index).getRating()<=65){
		...
		public static int[][] roundOutStatistics(Team team1, Team team2, int [] team1stats, int[] team2stats, boolean print_extra) {
			...
			if (team1stats[0]>team2stats[0]) {
				team1stats[2]+=rand.nextInt(3);
				printWinner(team1,team2,team2stats[0],team1stats[0],print_extra);
			}
		...
		private static void getPlayerOfGame(Team team_given) {
			...
			System.out.println("\t The Player of the Game for the "+team_given.getTeamName()+" is " +best_performer.showGameStatistics());
		}
		...
		//Adds teams to the playoff arraylist and returns the playoff arraylist
		//number_teams determines how many teams a user can input
		//If a user gives a number of teams greater than 8 - allow the user to select teams based on conference
		private static ArrayList<Team> userInputPlayoffTeams(ArrayList<Team> playoff_array_list, int number_teams){
			//If more than 16 teams are wanted - prompt the user about conferences
			if (number_teams > 8) {
				System.out.println(" [0] Playoffs By Conference, [1] Playoffs neglecting Conference ");
				System.out.print("You:");
				int choice = scan.nextInt();
				if (choice == 0) {
					//Get teams in eastern conference
					System.out.println(" Select Eastern Conference Team: ");
					ArrayList<Team> eastern_teams = userInputPlayoffsAll(playoff_array_list,number_teams/2,"EAST");
					//Get teams in western conference
					System.out.println(" Select Western Conference Team: ");
					ArrayList<Team> western_teams = userInputPlayoffsAll(playoff_array_list,number_teams/2,"WEST");
					//Add both conference teams to final array and return it
					ArrayList<Team> final_teams = new ArrayList<Team>();
					final_teams.addAll(eastern_teams);
					final_teams.addAll(western_teams);
					return final_teams;
				}
			}
			//If less than 16 teams are wanted - do not take in account conferences
			return userInputPlayoffsAll(playoff_array_list,number_teams,"");
		}
		
		//Adds teams to the playoff arraylist and returns the playoff arraylist
		//number_teams determines how many teams a user can input
		//if no conference is given - the Conferences of the teams selected does not matter
		//If the conference is given - only teams in that conference can be added to playoff_array_list
		private static ArrayList<Team> userInputPlayoffsAll(ArrayList<Team> playoff_array_list,int number_teams,String conference_given){
			int [] current_seeds = generateTeamSeedsNumber(number_teams);
			//Add a given number of teams to the playoff arraylist
			for( int team_counter =0; team_counter< number_teams; team_counter++) {
				//Shows the teams
				if (conference_given.length()>0) {
					//Only the the teams of a particular given conference if a conference is given
					NBACommonFunctions.showTeams(teamcounter,conference_given);
				}
				else {
					//Show all the teams if no conference is given
					NBACommonFunctions.showTeams(teamcounter);
				}
				//User picks the team
				System.out.print(current_seeds[team_counter]+".");
				int user_choice = scan.nextInt();
				if (NBACommonFunctions.checkValidityOfTeam(teamcounter,user_choice)) {
					Team team_selected  = teamcounter.get(user_choice);
					if (conference_given.length()>0&&(!team_selected.getConference().equals(conference_given))) {
						//If the conference given is not equal to the team's conference - inform user
						System.out.println(team_selected.getTeamName()+" is not in the "+conference_given);
						//Reset the counter
						team_counter--;
					}
					else {
						//If no conference is given or player is in the conference given
						//Either add team to playoffs or if duplicate inform user
						if (playoff_array_list.contains(team_selected)) {
							//If the team is already in the playoffs
							System.out.println(team_selected.getTeamName()+" is already in the playoffs");
							//Reset the counter
							team_counter--;
						}
						else {
							//If the team is not already in the playoffs add the team to the playoff arraylist
							playoff_array_list.add(team_selected);
							team_selected.setSeed(current_seeds[team_counter]);
						}
					}
				}
				else {
		...
		private static int[] generateTeamSeedsNumber(int number_teams) {
			if (number_teams == 2) {
				int[] final_array = {1,2};
				return final_array;
			}
			else if (number_teams == 4) {
				int[] final_array = {1,4,2,3};
				return final_array;
			}
			else if (number_teams == 8) {
				int[] final_array = {1,8,4,5,2,7,3,6};
				return final_array;
			}
			else {
				int[] final_array = {1,16,8,9,4,13,5,12,2,15,7,10,3,14,6,11};
				return final_array;
			}
		}
		...
		private static void runPlayoffSeries(Team top_team, Team bottom_team) {
			...
			System.out.println("\n"+top_team.playoffString());
			System.out.println(bottom_team.playoffString());
			//Reset the stats of the team for the matchup
			top_team.clearGames();
			bottom_team.clearGames();
			//Clears the player's statistics
			top_team.playerclearAll();
			bottom_team.playerclearAll();
			...
			while (top_team.getWins() < 4 && bottom_team.getWins() < 4){
				...
				if (games_played == 1 || games_played == 2 || games_played == 5 || games_played == 7) {
		...
		//Informs the user that injures are being healed if they wish to heal injuries
		...
		private static void showFullStandings(boolean heal_injured) throws IOException {
			if (heal_injured) {
				System.out.println("Healing Injures");
			}
		...
		private static void CompleteSeasonSim() throws IOException{
			...
			//Prompt the user to select whether they want playoffs to be formatted by conference or by overall standings
			System.out.println("[0] Seed Teams By Conference, [1] Seed Teams by Standings");
			ArrayList<Team> playoff_seeding;
			choice = scan.nextInt();
			if (choice == 1) {
				//transitions from the Season to the Playoffs
				playoff_seeding = byStandingsToPlayoffs();
			}
			else {
				playoff_seeding = byConferenceToPlayoffs();
			}
		...
		//Finds the 8 teams with highest record  in both conference and adds them to the final seeds arraylist
		//Returns a teamarraylist of the teams in bracket order i.e. "2nd Place" v.s. "7th Place" 
		//Eastern Teams first, Western Teams seconds (Order 1st v.s. 8th, 4th v.s. 5th, 2nd v.s. 7th, 3rd v.s. 6th)
		private static ArrayList<Team> byConferenceToPlayoffs(){
			ArrayList<Team> eastern =seedTeamsByConference("EAST");
			ArrayList<Team> western =seedTeamsByConference("WEST");
			//Adds both conferences to the final array "East first then west"
			ArrayList<Team>  finalseeds = new ArrayList<Team>();
			finalseeds.addAll(eastern);
			finalseeds.addAll(western);
			return finalseeds;
		}
		
		//Finds the 8 best record teams in the given conference and adds the to the final conference_seeding arraylist
		//Returns the teamarraylist of the teams in bracket order i.e. "3rd Place" v.s. "6th Place" 
		private static ArrayList<Team> seedTeamsByConference(String conference_given) {
			ArrayList<Team> rounds = new ArrayList<Team>();
			for (int playoff_spots = 0; playoff_spots < 8; playoff_spots++) {
				//Find the 16 best teams in the league (No matter conference)
				int current_team_index = 0;
				Team current_team = teamcounter.get(current_team_index);
				while (rounds.contains(current_team)){
					//Find the next team not already in the playoffs
					current_team_index++;
					current_team = teamcounter.get(current_team_index);
				}
				//Find the team with the next highest wins who is not already in the playoffs
				for (int new_team_index = current_team_index+1; new_team_index  <teamcounter.size();new_team_index ++) {
					//If the team is not already in the playoffs
					if ((!rounds.contains(teamcounter.get(new_team_index )))&&teamcounter.get(new_team_index).getConference().equals(conference_given)){
						//If the team has a higher number of wins than current highest team - make team current highest win team
						if (current_team.getWins() < teamcounter.get(new_team_index ).getWins()){
							current_team = teamcounter.get(new_team_index );
						}
					}
				}
				current_team.setSeed(playoff_spots+1);
				rounds.add(current_team);
			}
			//Seeds the team so that the best teams in the playoffs are playing the worst teams i.e. "1st Place" v.s. "8th Place"
			//Final order will be 1st v.s. 8th, 4th v.s. 5th, 2nd v.s. 7th, 3rd v.s. 6th
			ArrayList<Team> conference_seeding = new ArrayList<Team>();
			while (rounds.size() > 0) {
				conference_seeding.add(rounds.remove(0));
				conference_seeding.add(rounds.remove(rounds.size()-1));
				int half_length = (rounds.size() / 2);
				if (half_length > 2) {
					conference_seeding.add(rounds.remove(half_length-1));
					conference_seeding.add(rounds.remove(half_length-1));
				}
			}
			return conference_seeding;
		}
		...
		private static ArrayList<Team> byStandingsToPlayoffs(){
			ArrayList<Team> rounds = new ArrayList<Team>();
			for (int playoff_spots = 0; playoff_spots < 16; playoff_spots++) {
				...
				for (int new_team_index = current_team_index+1; new_team_index  <teamcounter.size();new_team_index ++) {
					//If the team is not already in the playoffs
					if (!rounds.contains(teamcounter.get(new_team_index ))){
						//If the team has a higher number of wins than current highest team - make team current highest win team
						if (current_team.getWins() < teamcounter.get(new_team_index ).getWins()){
							current_team = teamcounter.get(new_team_index );
						}
			...
			ArrayList<Team> playoff_seeding = new ArrayList<Team>(rounds.size());
			while (rounds.size() > 0) {
				 playoff_seeding.add(rounds.remove(0));
				 playoff_seeding.add(rounds.remove(rounds.size()-1));
				 int overall_iteration = 1;
				 int half_length = (rounds.size() / 2);
				 //If there are going to be more than 4 teams, create two seperate brackets so that 
				 //1st and 2nd seed teams can face in the Finals
				 while (half_length > 2) {
					 int current_iteration = overall_iteration;
					//Add all the appropriate matchups for the current iteration
					 while (current_iteration >= 1) {
						playoff_seeding.add(rounds.remove(half_length-1));
						playoff_seeding.add(rounds.remove(rounds.size()-half_length));
						current_iteration--;
					 }
					 //Recursive call for the bracket
					 half_length = (half_length / 2);
					 //If every additional team add - 2 additional teams should be added for
					 overall_iteration*=2;
				 }
			}
		...
		private static void setPlayersGamesPlayed(Team team_given,int team_rotation, boolean show_special_boxscore) {
			...
			for (int player_index=0;player_index<team_rotation;player_index++) {
				if (!previous_player.equals(best_player)){
					...
					//Print Special Box Score if Player played extraordinarily
					if (best_player.getPoints()+best_player.getRebounds()+best_player.getAssists()>9) {
		...
		private static void calculateBoxScorePoints(Team team_given, int team_rotation,int total_points, boolean isPlayoffs) {
			...
			for (int player_in_rotation=0;player_in_rotation<team_rotation;player_in_rotation++) {
				//If the loop runs longer than the rotation - end loop
				checkRunTime++;
				if (checkRunTime >= team_given.leng()) {
					player_in_rotation=team_rotation;
				}
				else {
					//If there are significant amount of points left and not last player in rotation
					if (total_points > 5 || player_in_rotation<team_rotation-1) {
						//Make sure the player does not already have value set and they are points left
						if (best_scorer.getPoints()==0 && best_scorer.getRating() >65) {
							current_player_points = rand.nextInt((int) (best_scorer.getRating()/7)+1) + rand.nextInt((int) (best_scorer.getRating()/7)+1) - rand.nextInt((int) (best_scorer.getScoring()+5+team_given.getBestPlayer().getRating()-best_scorer.getRating()));
							current_player_points += rand.nextInt(Math.incrementExact(Math.abs(total_points/(((team_rotation-player_in_rotation)+4+player_in_rotation)/2))));
							current_player_points += rand.nextInt(1+(total_points/25)*best_scorer.getScoring())+((((best_scorer.getRating()-team_given.getAvg()+best_scorer.getScoring()))/4) - rand.nextInt(checkRunTime));
							//If the player is an allstar or is in the top 3 in PPG within their team
							if (best_scorer.getAllstar()||best_scorer.getScoring()<4) {
								//If it is the points
								if (isPlayoffs) {
									current_player_points += rand.nextInt(3);
								}
								current_player_points+=rand.nextInt((int) (best_scorer.getRating()/(24+best_scorer.getScoring()))+1);
							}
							//If the player is an rookie
							if(best_scorer.getRookieStatus()&&current_player_points>25) {
							...
							//If the player scored the rest of the points
							if (current_player_points >= total_points) {
								best_scorer.setPoints(total_points);
								total_points = 0;
								player_in_rotation = team_rotation;
							}
							//If there are still points left to be accounted for
							else {
							...
							//Reset the counter if player has already been calculated
							player_in_rotation--;
						}
					...
					//If there are only a few points or last player in rotation 
					//they assign them to the rest of the points and exit the loop
					else if (total_points > 0 && total_points < 5 || player_in_rotation<team_rotation-1) {
			...
			for (int player_in_rotation=0;player_in_rotation<team_rotation;player_in_rotation++) {
				...
				if (checkRunTime >= team_given.leng()) {
					...
					if (!best_rebounder.getInjury()) {
						if (total_rebounds>0 && best_rebounder.getRating() >65) {
							current_player_rebounds = ((int) ((best_rebounder.getRating()*3) - (Math.max(65, team_given.getAvgC())+Math.max(65, team_given.getAvgPF())+99)/2) / (best_rebounder.getRebounding()  +  35));
							current_player_rebounds += rand.nextInt(1+(total_rebounds/(10*(player_in_rotation+1)))) - rand.nextInt(checkRunTime/(player_in_rotation+1));
							if(current_player_rebounds < 0) {
								current_player_rebounds = 0;
							}
							current_player_rebounds += rand.nextInt(1+(int)(total_rebounds/((best_rebounder.getRating()+checkRunTime)/(team_rotation+checkRunTime))));
							//Make sure the player does not already have value set
							if (best_rebounder.getRebounds()==0) {
								//If the player is an allstar and is in the top 5 in RPG within their team
								if (best_rebounder.getRebounding()<6) {
									//If it is the playoffs
									if (isPlayoffs) {
										current_player_rebounds += rand.nextInt(3);
									}
									current_player_rebounds+=rand.nextInt((int)(best_rebounder.getRating()*(team_rotation -best_rebounder.getRebounding())/total_rebounds)/2);
								}
								//If not a center of power forward - lower rebounds (compensate for height)
								if (!(best_rebounder.getPosition().equals("C")||best_rebounder.getPosition().equals("PF"))) {
									current_player_rebounds-=rand.nextInt(Math.min(6,2+((current_player_rebounds+team_rotation+player_in_rotation)/(player_in_rotation+1))));
								}
								//If the player is a rookie
								if(best_rebounder.getRookieStatus()&&current_player_rebounds>=12) {
									current_player_rebounds-=rand.nextInt(5);
								}
								if (current_player_rebounds<0) {
									current_player_rebounds=0;
								}
								if (current_player_rebounds>=total_rebounds) {
									current_player_rebounds =total_rebounds;
								}
							...
							//If the player has already been given rebounds
							else {
		...
		private static void calculateBoxScoreAssists(Team team_given, int team_rotation,int total_assists, boolean isPlayoffs) {
			...
			for (int player_in_rotation=0;player_in_rotation<team_rotation;player_in_rotation++) {
				...
				if (checkRunTime >= team_given.leng()) {
					...
					//If there are still assists left to be account for
					if (total_assists > 0) {
						//If the player is not injured
						if (!best_assister.getInjury()) {
								if (best_assister.getAssists()==0 && best_assister.getRating() >65 ) {
									current_player_assists = ((int) ((best_assister.getRating()*3) - (Math.max(65, team_given.getAvgPG())+Math.max(65, team_given.getAvgSG())+99)/2) / (best_assister.getAssisting()  +  75));
									if(current_player_assists < 0) {
										current_player_assists = 0;
									}
									current_player_assists += rand.nextInt(1+(total_assists/(25*(player_in_rotation+1)))) - rand.nextInt(checkRunTime/(player_in_rotation+1));
									current_player_assists += rand.nextInt(1+(int)(total_assists/(best_assister.getRating()/(team_rotation+player_in_rotation))));
									//If the player is an allstar and is in the top 2 in APG within their team
									if (best_assister.getAssisting()<3) {
										//If it is the playoffs boost the stats
										if (isPlayoffs) {
											current_player_assists += rand.nextInt(3);
										}
										current_player_assists+=(total_assists/(best_assister.getRating()/8));
										...
										if (best_assister.getAssisting()==1||best_assister.getPosition().contains("G")) {
											current_player_assists+=rand.nextInt(4-best_assister.getAssisting());
										}
									}
									if (current_player_assists<0) {
										current_player_assists=0;
									}
									if (current_player_assists>=total_assists) {
										current_player_assists =total_assists;
									}
									...
									//Reset the counter if player is not in rotation or already been accounted for
									player_in_rotation--;
								}
						}
						else {
		...
		private static Player bestScorer(Team team_given,int rank_given, Player player_given) {
			for(int player_index=0;player_index<team_given.leng();player_index++) {
				if (team_given.getPlayer(player_index).getScoring()==rank_given) {
					...
					if (!team_given.getPlayer(player_index).getInjury()) {
		...
		private static void matchups(Team team_given) {
			...
			for (int position_index=0;position_index<position_chart.length;position_index++) {
				...
				for (int player_index=0;player_index<team_given.leng();player_index++) {
					//If the player is not injured
					if (team_given.getPlayer(player_index).getInjury()==false && team_given.getPlayer(player_index).getRating() > 65) {
