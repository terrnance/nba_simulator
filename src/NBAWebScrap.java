...
public class NBAWebScrap{
	...
	//Gets player who are allstars and makes them allstars
	//Gets player who are injured and makes them injured
	public static void getLatestUpdates() throws IOException{
		...
		//Scraps the internet to get the current allstars
		getAllstarsScrap();
	...
	//If name given matches a player's name make the player an allstar
	//Finds the player and makes them an allstar
	private static void makePlayerAllstar(String allstars_name) {
		//Find the player whose name was given and make that person an allstar
		for (int player_index =0 ;player_index<latest_roster_players.size();player_index++) {
			//Once found set Allstar status to true
			if (latest_roster_players.get(player_index).getPlayerName().equals(allstars_name)) {
				latest_roster_players.get(player_index).setAllstar(true);
	...
	//Called by getLatestUpdates()
	//Reads the HTML from the URL: "https://www.cbssports.com/nba/injuries/"
	//Finds the line of code containing the injured player and their name
	//Sets the player's injury status to true Makes the player injured
	private static void getInjuriesScrap() throws IOException {
		URL url = new URL("https://www.cbssports.com/nba/injuries/");
        // Get the input stream through URL Connection
        URLConnection connection = url.openConnection();
        InputStream inputstream = connection.getInputStream();
        BufferedReader br = new BufferedReader(new InputStreamReader(inputstream));
        ...
        // Find the line containing the list of players and save it
        while ((line = br.readLine()) != null && continue_loop) {
        		if (line.contains("playerpage")) {
        			String player_name = extractInjuredPlayer(line);
        			makePlayerInjured(player_name);
        		}
        }
	}
	
	//Called by getInjuriesScrap()
	//Given a HTML line containing a list of all the injured players - call a regular expression
	//to get only the player's name
	private static String extractInjuredPlayer(String string_given) {
		String[] new_list = string_given.split("<a href=\"\\/nba\\/players\\/playerpage\\/[0-9]*\\/[a-z]*-[a-z]*[-]*[a-z]*\" class=\"\">|<\\/a>");
		return new_list[1];
	}
	//If name given matches a player's name make the player injured
	//Finds the player and makes them an injured
	private static void makePlayerInjured(String players_name) {
		//Find the player whose name was given and make that person injured
		for (int player_index =0 ;player_index<latest_roster_players.size();player_index++) {
			//Once found set Injured status to true
			if (latest_roster_players.get(player_index).getPlayerName().equals(players_name)) {
				latest_roster_players.get(player_index).setInjury(true);
				player_index = latest_roster_players.size();
			}
			//Special Case because Nene goes by two names sometimes
			if (players_name.equals("Nene")) {
				if (latest_roster_players.get(player_index).getPlayerName().equals("Nene Hilario")) {
					latest_roster_players.get(player_index).setInjury(true);
					player_index = latest_roster_players.size();
				}
			}
	...
	private static void readDirectory(String directory_name) throws IOException {
